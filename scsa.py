...
class SCSA(ABC):
    ...
    def write_to_file(self, codes: list[str], length: int, num_colors: int) -> None:
        ...
        file_name = self.name + "_" + str(length) + "_" + str(num_colors) + "_"+ str(len(codes)) + ".txt"
...
# Mystery 2 SCSA
class Mystery2(SCSA):
    """SCSA that chooses 4 colors randomly and repeats this pattern"""

    def __init__(self):
        """Constructor for Mystery2"""
    ...
    def generate_codes(
        ...
        """Generate codes based on Mystery SCSA
        ...
        for _ in range(num_codes):
            pattern = list_to_str(random.sample(colors, k=4))
            code = (pattern * (length // 4)) + pattern[:length % 4]
            codes.append(code)
...
# Mystery 5 SCSA
class Mystery5(SCSA):
    """SCSA that chooses 2 colors and follows xyxyy pattern"""

    def __init__(self):
        """Constructor for Mystery5"""
    ...
    def generate_codes(
        ...
        for _ in range(num_codes):
            ...
            for i in range(length):
                if i % 5 == 0 or i % 5 == 2:
                    code.append(first_color)
                else:
                    code.append(second_color)
